<template>
  <view class="pc-tab-list">

    <!-- Tab吸顶 -->
    <u-sticky style="top: 12px">
      <view class="head-tab" style="width: 250px; margin-left: 150px;">
        <u-tabs
            :list="tabList"
            lineColor="#72CD18"
            lineWidth="46"
      ...
      <!-- 个人图标 -->
      <view class="login-icon" @click="showPcSNSLogin">
      ...
      <view v-if="showAbout" class="about-icon" @click="aboutPopupShow = true">
        <u-icon name="info-circle-fill" size="24"></u-icon>
      </view>
      <!-- 关于图标 -->
    </u-sticky>
    <!-- Tab吸顶 -->
    ...
    <!-- 移动端 登录/个人弹出框 -->
    <u-popup :show="PCDeviceShow" mode="center" :closeable="true" closeIconPos="top-left" @close="closePop">
      <view class="pop-login-list">
        <view style="padding: 0 30px;">

          <view style="text-align: center; font-size: 15px; font-weight: 400; margin-bottom: 10px;margin-top: 13px">
            <span v-if="isLogin">プロフィール</span>
            <span v-else>ソーシャルログイン</span>
          </view>

          <view class="sns-icon-area" v-if="!isLogin" style="margin-top: 15px">
            <!--Google Icon-->
          ...
          <view v-else>
            ...
            <view class="sns-login-style sns-login-top" style="height: 40px" @click="logout">
              <view class="sns-login-style-svg">
...
export default {
  ...
  methods: {
    showMobileSNSLogin() {
      this.PCDeviceShow = true
    },
    showPcSNSLogin() {
      this.PCDeviceShow = true
    },
    closePop() {
      this.PCDeviceShow = false
    },
    logout() {
    ...
    twitterLogin() {
      ...
      window.location.href = config.twitterAuthenticationUrl + '?page=' + uni.$u.page()  + '&system_id=' + systemInfo.deviceId
    },
    facebookLogin() {
